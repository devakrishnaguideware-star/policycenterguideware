...
class PCSolrMessageTransportPlugin extends AbstractSolrMessageTransportPlugin implements ISolrMessageTransportPlugin  {
  ...
  var _consistencyCacheSize = new AtomicReference<Integer>()
  var _consistencyCacheExpirationSeconds = new AtomicReference<Integer>()

  construct() {
    this("PCSolrMessageTransportPlugin")
  }
  ...
  static function checkEnabled() {
    if (not Enabled) {
      throw new DisplayableException(DisplayKey.get("SolrMessageTransportPlugin.Error.NotEnabled"))
    }
  }
  ...
  override function send(message : Message, data : String) {
    ...
    try {
      ...
      for(solrMsg in solrMsgList) {
        ...
        var response = updateRequest?.process(PCDependenciesGateway.findSolrClient(solrMsg.DocType))
  ...
  override function parseMessagePayload(messagePayload: String): List<SolrUpdateMessage> {
    return PCSolrMessageAggregator.parse(messagePayload)
  }
    ...
    switch(typeof bean){
      case PolicyPeriod:
        if (bean.Policy.Account == solrContext.Account) {
          ...
          if (bean.Preempted or (PolicyPeriodStatus.TF_INELIGIBLEFORSOLRINDEX.TypeKeys.contains(bean.Status))) {
            processDeletion(bean, solrContext)
          } else {
  ...
  protected function createMessages(account : Account, messageContext : MessageContext, requestList : List<IMessageRequest>) {
    ...
    if(msg?.HasContent) {
      ...
      // Similarily to PolicyPeriod, null out Policy for purge events to avoid FK constraint violation.
      // Potentially there can be also a FK conflict with PC_MESSAGEHISTORY pointing to the policy.
      // To avoid it, make sure that there are no historical message records pointing at the purged policy
      // by setting PruneVersionsRecentJobCompletionDays > KeepCompletedMessagesForDays
      if (messageContext.EventName == Policy.POLICYPURGED_EVENT) {
        newMessage.setFieldValue("Policy", null)
      }
