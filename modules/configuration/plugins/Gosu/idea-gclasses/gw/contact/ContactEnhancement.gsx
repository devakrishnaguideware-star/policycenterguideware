...
uses gw.api.util.ExternalIDUtil#mintNewExternalID(String)
uses gw.api.util.PCSequenceUtilKeyRegistry#CONTACT_EXTERNALID
uses gw.api.util.PCSequenceUtilKeyRegistry#ADDRESS_EXTERNALID
...
enhancement ContactEnhancement : entity.Contact {
  ...
  function safeRemoveAddress(address : Address) {
    if (this.addressIsPolicyAddressInUse(address)) {
      throw new DisplayableException(DisplayKey.get("Java.Error.Contact.CannotRemoveAddress", address))
    }
  ...
  function mergeWithContact(contact : Contact) {
    ...
    if (contact == null) {
      throw new IllegalArgumentException(DisplayKey.get("Contact.MergeContacts.Error.NullContactArgument"))
    }
    if (contact == this) {
      throw new IllegalArgumentException(DisplayKey.get("Contact.MergeContacts.Error.SameContactArgument"))
    }
    if (this.Subtype != contact.Subtype) {
      throw new IllegalArgumentException(DisplayKey.get("Contact.MergeContacts.Error.DifferentContactSubTypeArgument", this.Subtype, contact.Subtype))
    }
    ...
    contact.PrimaryAddress = null  // So that we can purge the victim contact
  ...
  public function mergeAddresses(survivingAddress : Address, mergedAddress : Address) {
    if (survivingAddress == null) {
      throw new IllegalArgumentException(DisplayKey.get("Contact.MergeAddresses.Error.NullSurvivorAddress"))
    }
    if (mergedAddress == null) {
      throw new IllegalArgumentException(DisplayKey.get("Contact.MergeAddresses.Error.NullMergedAddress"))
    }
    if (mergedAddress == survivingAddress) {
      throw new IllegalArgumentException(DisplayKey.get("Contact.MergeAddresses.Error.SurvivorAddressSameAsMergedAddress", survivingAddress))
    }
    if (mergedAddress == this.PrimaryAddress) {
      throw new IllegalArgumentException(DisplayKey.get("Contact.MergeAddresses.Error.MergedAddressCannotBePrimary", mergedAddress))
    }
    ...
    if (mergedContactAddress == null) {
      throw new IllegalArgumentException(DisplayKey.get("Contact.MergeAddresses.Error.MergedAddressNotOnContact", mergedAddress, this))
    }
    if (survivorContactAddress == null and survivingAddress != this.PrimaryAddress) {
      throw new IllegalArgumentException(DisplayKey.get("Contact.MergeAddresses.Error.SurvivorAddressNotOnContact", survivingAddress, this))
    }
  ...
  property get NewClaimSearchCriteria() : ClaimSearchCriteria {
    ...
    criteria.DateCriteria.DateSearchType = TC_ENTEREDRANGE
    return criteria
  }
  ...
  property get PolicyPeriods() : Collection<PolicyPeriod> {
    if (this.New) {
      ...
      query = new ViewablePolicyPeriodQueryFilter().filterNewQuery(query) as Query<PolicyPeriod>
      return query.select().toSet()
    }
  }
  
  static function findContactInBundleOrDB(contactUID : String, bundle : Bundle) : Contact {
    var foundContact = bundle.InsertedBeans.firstWhere(\ c -> c typeis entity.Contact && c.AddressBookUID == contactUID) as entity.Contact
    if (foundContact != null) {
      return foundContact
    }

    // next, look for updated beans in the current bundle
    foundContact = bundle.UpdatedBeans.toCollection().firstWhere(\ c -> c typeis entity.Contact && c.AddressBookUID == contactUID) as entity.Contact
    if (foundContact != null) {
      return foundContact
    }

    // see if we've deleted the contact - this might be an error
    foundContact = bundle.RemovedBeans.toCollection().firstWhere(\ c -> c typeis entity.Contact && c.AddressBookUID == contactUID) as entity.Contact
    if (foundContact != null) {
      return foundContact
    }
  ...
  static function findContactByPublicId(contactPublicID : String) : Contact {
    var foundContact = Query.make(Contact).compare(Contact#PublicID, Equals, contactPublicID).select().AtMostOneRow
  ...
  /**
   * @return An array of all the accounts on which this contact plays the specified {@code role}.
   */
  function getAccountsByRoleType(role: typekey.AccountContactRole): Account[] {
    return this.AccountContacts.where(\ac -> ac.hasRole(role)).map(\ac -> ac.Account)
  }

  function mintContactExternalIDIfNecessary() {
    if (not(this typeis UserContact) and this.IsLocalOnly) {
      this.ExternalID = mintNewExternalID(CONTACT_EXTERNALID)
    }
  }

  function mintPrimaryAddressExternalIDIfNecessary() {
    if (not(this typeis UserContact) and this.PrimaryAddress.NeedsNewExternalID) {
      this.PrimaryAddress.ExternalID = mintNewExternalID(ADDRESS_EXTERNALID)
    }
  }
}
