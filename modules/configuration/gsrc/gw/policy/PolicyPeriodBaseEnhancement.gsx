...
enhancement PolicyPeriodBaseEnhancement: PolicyPeriod {
  ...
  public property get PolicyNumberDisplayString(): String {
    return this.PolicyNumberAssigned ? this.PolicyNumber : DisplayKey.get("EntityName.PolicyPeriod.PolicyNumber.Unassigned")
  }
  ...
  property get UnderWriterIssueBlockingPoint(): UWIssueBlockingPoint {
    switch (true) {
      case this.Status == TC_BOUND:
        return TC_NONBLOCKING

      case this.ValidQuote and !this.QuoteHidden:
        return TC_BLOCKSBIND

      case (this.ValidQuote and this.QuoteHidden) or (this.ValidRate and !this.QuoteHidden):
        return TC_BLOCKSQUOTERELEASE

      case this.ValidRate and this.QuoteHidden:
        return TC_BLOCKSRATERELEASE

      default:
        return TC_BLOCKSQUOTE
    }
  }
  ...
  function checkBaseEditability(): boolean {
    ...
    if (wrongSlice and not this.FailedOOSEValidation and (this.SliceDate == null or isOOSSlice())) {
  ...
  private static function compareNullable<T extends Comparable<T>>(one: T, two: T): int {
  ...
  private function compareInModelTime(that: PolicyPeriod): int {
    if (this == that) {
      return 0
    }
    var termDiff = compareNullable(this.TermNumber, that.TermNumber)
    return termDiff != 0 ? termDiff : compareNullable(this.ModelNumber, that.ModelNumber)
  }

  private function isOOSSlice(): boolean {
    for (pp in this.Policy.Periods) {
      if (compareInModelTime(pp) > 0 and pp.EditEffectiveDate.after(this.EditEffectiveDate) and pp.isEffective(this.SliceDate)) {
        return true
      }
    }
    return false
  ...
  public static function getPeriodDisplayStatus(asOf: Date, periodStatus: PolicyPeriodStatus,
    ...
    if (periodStatus != TC_BOUND) {
      ...
      return DisplayKey.get("PolicyPeriod.PeriodDisplayStatus.Scheduled")
    } else if (asOf >= periodEnd) {
      return DisplayKey.get("PolicyPeriod.PeriodDisplayStatus.Expired")
    } else {
      return DisplayKey.get("PolicyPeriod.PeriodDisplayStatus.InForce")
    }
  }
   ...
   * are active
   *
   * @param includeNonSpecificLocations
   */
  function getUnassignedAccountLocations(includeNonSpecificLocations: boolean): List<AccountLocation> {
    var unassignedAccountLocations = this.getUnassignedAccountLocations()
    if (not includeNonSpecificLocations) {
      unassignedAccountLocations = unassignedAccountLocations.where(\l -> not l.NonSpecific)
    }
  ...
  function runSubmissionIssuanceLogic() {
    this.Job.createCustomHistoryEvent(TC_SUB_ISSUED, \-> DisplayKey.get("Submission.History.JobIssued"))
  ...
  function edit() {
    ...
    clearDenormalizedFinancialTotals()
    this.TaxSurchargesRPT = null
  ...
  function rollbackToRated() {
    this.Status = TC_RATED
    this.QuoteHidden = false  //quote is never hidden in rated
    // clear selected payment plan -- allows quote process to update billing correctly.
    this.JobProcess.cleanUpAfterEdit()
  }
  ...
  property get CanUpdatePeriodDates(): boolean {
    return not this.Promoted and this.Job?.CanUpdatePeriodDates and (this.Status == TC_NEW or this.Status == TC_DRAFT)
  }
  ...
  property get CanCopyCoverages(): boolean {
    return this.Job?.CanCopyCoverages and (this.Status == TC_NEW or this.Status == TC_DRAFT)
  }
  ...
  function getNewClaimSearchCriteria(useThisPolicy: boolean): ClaimSearchCriteria {
    ...
    criteria.PolicyNumber = this.PolicyNumber
    if (useThisPolicy) {
  ...
  private function ensureProducerIsValid() {
    var producer = this.Job.getUserRoleAssignmentByRole(TC_PRODUCER)
    ...
    if (producer != null && not this.Policy.ProducerCodeOfService.isAvailableFor(producer.AssignedUser)) {
      ...
      var policyProducer = this.Policy.getUserRoleAssignmentByRole(TC_PRODUCER)
      if (producer.AssignedUser == policyProducer.AssignedUser) {

        this.Policy.removeFromRoleAssignments(policyProducer)
        var newProducer = this.Job.getUserRoleAssignmentByRole(TC_PRODUCER)
        AssignmentUtil.assignAndLogUserRole(this.Policy, newProducer.AssignedUser, newProducer.AssignedGroup, TC_PRODUCER,
  ...
  function getWizardWarningMessages(): List<String> {
    ...
    if (this.hasAnyUnhandledPreemptions() and not this.Locked) {
      ...
      result.add(DisplayKey.get("Web.Job.Preempted.Warning", this.Job.DisplayType, jobs))
    }
    ...
    if (this.Job.OOSJob and not this.Locked) {
      if (this.Canceled and this.EditEffectiveDate.before(this.CancellationDate)) {
        ...
        result.add(DisplayKey.get("Web.Job.OOS.CancelWarning", this.Job.DisplayType, dateStr))
      } else {
        var oosDates = this.OOSSliceDates.where(\d -> d.after(this.EditEffectiveDate)).map(\d -> d.format("short")).join(", ")
        result.add(DisplayKey.get("Web.Job.OOS.Warning", this.Job.DisplayType, oosDates))
      }
    }

    if (this.FailedOOSEValidation) {
      result.add(DisplayKey.get("Web.Job.OOS.OOSEValidationWarning"))
    } else if (this.FailedOOSEEvaluation) {
      result.add(DisplayKey.get("Web.Job.OOS.OOSEEvaluationWarning"))
    }

    if (this.Job typeis Renewal) {
      if (this.Renewal.hasAnotherPendingPeriod(this)) {
        result.add(DisplayKey.get("Web.RenewalWizard.OtherPeriod.Message"))
      }
      var prevTermChanged = this.Policy.Periods
          .hasMatch(\p -> p.ResolveWithFuturePeriods and this.BasedOn.TermNumber == p.TermNumber)
      if (prevTermChanged) {
        result.add(DisplayKey.get("Web.RenewalWizard.PriorTerm.Message"))
      }
    }
    ...
    if (this.Cancellation != null and
      ...
      if (scheduledCancellation != null) {
        result.add(DisplayKey.get("Web.Cancellation.Error.AlreadyScheduled", scheduledCancellation.JobNumber))
      }
    }

    if (!this.Locked and this.BasedOn != null and this.BasedOn.IsArchivedInBillingSystem) {
      result.add(DisplayKey.get("Web.Job.BillingSystem.Archived.Warning"))
    }


    // Warn if the policy has outstanding contingencies
    result.addAll(this.Policy.ContingencyWarningMessages)
  ...
  function getWizardQuoteScreenWarnings(): List<String> {
    ...
    if ((perm.System.viewratingoverrides or perm.System.editratingoverrides) and this.hasAtLeastOneCostOverride()) {
      result.add(DisplayKey.get("Web.QuoteScreen.HasOverridesWarning"))
    }
  ...
  function createTimeoutWorkflow(wakeupTime: Date, functionToCall: String): TimeoutWF {
    var workflow = this.createWorkflow(TC_TIMEOUTWF) as TimeoutWF
  ...
  function cancelTimeout() {
    ActiveTimeoutWorkflows.each(\workflow -> workflow.invokeTrigger(TC_CANCEL))
  }
  ...
  private function makeLicensedStateQuery(uwCompanies: UWCompany[]): Query<LicensedState> {
    ...
    qry.compare("ProductCode", Equals, this.Policy.Product.PublicID)
   ...
   */
  function getUWCompanyRateFactor(ratingEffectiveDate: Date, state: Jurisdiction): Double {
    // Map: (UWCompany.PublicID, Product.Code, State, Date) -> PriceFactor
    // NOTE: It's important that this is a SessionVar so it's value is preserved
    // across calls to this function within the same user session.
    var priceFactorCache: SessionVar<Map<String, Double>> = PriceFactorCache.Instance

    // Initialize the cache if necessary
    if (priceFactorCache.RequestAvailable && priceFactorCache.get() == null) {
      priceFactorCache.set(new HashMap<String, Double>())
    }

    // Set some useful values
    var comp = this.UWCompany
    var product = this.Policy.Product.PublicID
    ...
    if (priceFactorCache.RequestAvailable) {
      priceFactor = priceFactorCache.get().get(key)
      if (priceFactor != null) {
        return priceFactor
      }
    }
    ...
    if (not queryIterator.hasNext()) {
      ...
      PCLoggerCategory.JOB_PROCESS.warn(DisplayKey.get("PolicyPeriod.UWCompany.NoLicensedStates", comp.DisplayName, product, state, ratingEffectiveDate))
      priceFactor = 1.0
    } else {
      var licensedState = queryIterator.next()
      if (queryIterator.hasNext()) {
        // warn that this should not happen and throw exception to the UI
        var errMsg = DisplayKey.get("PolicyPeriod.UWCompany.MultipleApplicableRatingFactors", comp.DisplayName, product, state, ratingEffectiveDate)
    ...
    if (priceFactorCache.RequestAvailable) {
      priceFactorCache.get().put(key, priceFactor)
    }
    return priceFactor
  }
  ...
  property get JobCompletionMessage(): String {
    switch (this.Status) {
      case TC_WITHDRAWN:
        return DisplayKey.get("Web.JobComplete.Message.Withdrawn", this.Job.DisplayType, this.Job.JobNumber)
      case TC_DECLINED:
        return DisplayKey.get("Web.JobComplete.Message.Declined", this.Job.DisplayType, this.Job.JobNumber)
      case TC_NOTTAKING:
        return DisplayKey.get("Web.JobComplete.Message.NotTaking", this.Job.JobNumber)
      case TC_NOTTAKEN:
        return DisplayKey.get("Web.JobComplete.Message.NotTaken", this.Job.DisplayType, this.Job.JobNumber)
      case TC_BINDING:
        return DisplayKey.get("Web.JobComplete.Message.Binding", this.Job.DisplayType, this.Job.JobNumber)
      case TC_BOUND:
        return DisplayKey.get("Web.JobComplete.Message.Bound", this.Job.DisplayType, this.Job.JobNumber)
      case TC_REINSTATING:
        return DisplayKey.get("Web.JobComplete.Message.Reinstating", this.Job.DisplayType, this.Job.JobNumber)
      case TC_NONRENEWING:
        return DisplayKey.get("Web.JobComplete.Message.NonRenewing", this.Job.JobNumber)
      case TC_NONRENEWED:
        return DisplayKey.get("Web.JobComplete.Message.NonRenewed", this.Job.JobNumber)
      case TC_RENEWING:
        return DisplayKey.get("Web.JobComplete.Message.Renewing", this.Job.JobNumber)
      case TC_RESCINDED:
        return DisplayKey.get("Web.JobComplete.Message.Rescinded", this.Job.JobNumber)
      case TC_CANCELING:
        return DisplayKey.get("Web.JobComplete.Message.ScheduledCancellation", this.Job.JobNumber, DateFormat.getDateTimeInstance().format(this.CancellationDate))
      case TC_AUDITCOMPLETE:
        return DisplayKey.get("Web.JobComplete.Message.AuditComplete", (this.Job as Audit).AuditInformation.AuditScheduleType, this.Job.JobNumber)
      case TC_WAIVED:
        return DisplayKey.get("Web.JobComplete.Message.AuditWaived", this.Job.JobNumber)
      case TC_DRAFT:
        return DisplayKey.get("Web.JobComplete.Message.Draft", this.Job.DisplayType, this.Job.JobNumber)
      case TC_RATED:
        return DisplayKey.get("Web.JobComplete.Message.Rated", this.Job.DisplayType, this.Job.JobNumber)
      case TC_QUOTED:
        return DisplayKey.get("Web.JobComplete.Message.Quoted", this.Job.DisplayType, this.Job.JobNumber)
      default:
        return DisplayKey.get("Web.JobComplete.Message.UnknownStatus", this.Status)
    }
  }
  ...
  property get JobCompletionTitle(): String {
    switch (this.Status) {
      case TC_REINSTATING:
        return DisplayKey.get("Web.JobComplete.Title.Reinstating", this.Job.DisplayType)
      case TC_CANCELING:
        return DisplayKey.get("Web.JobComplete.Title.Canceling", this.Job.DisplayType)
      default:
        return DisplayKey.get("Web.JobComplete.Title", this.Job.DisplayType, this.Status)
    }
  }
  ...
  function checkPolicyTermAgainstProductModel(): List<ProductModelSyncIssueWrapper> {
    return PCProfilerTag.PRODUCT_MODEL_SYNC.evaluate(\p -> {
      p.setPropertyValue("SyncType", PCProfilerTag.ProductModelSyncType.PolicyTerm.name());
      p.setPropertyValue("Offering", this.EffectiveDatedFields.OfferingCode)
      ...
      p.setPropertyValue("NumIssues", String.valueOf(originalIssues.size()));
  ...
  property get UnassignedAdditionalNamedInsureds(): AccountContact[] {
    ...
    var accountContactRoleType = plugin.getAccountContactRoleTypeFor(TC_POLICYADDLNAMEDINSURED)
    var assignedPolicyNamedInsureds = this.NamedInsureds.map(\ni -> ni.AccountContactRole.AccountContact)
  ...
  property get AdditionalNamedInsuredOtherCandidates(): AccountContact[] {
    var plugin = Plugins.get(IContactConfigPlugin)
    var accountContactRoleType = plugin.getAccountContactRoleTypeFor(TC_POLICYADDLNAMEDINSURED)
    var otherAccountContacts = this.Policy.Account.getAccountContactsWithAnyRole({TC_ACCOUNTHOLDER, TC_DRIVER, TC_SECONDARYCONTACT})
  ...
  function addNewPolicyAddlNamedInsuredForContact(contact: Contact): PolicyAddlNamedInsured {
    return this.addNewPolicyContactRoleForContact(contact, TC_POLICYADDLNAMEDINSURED) as PolicyAddlNamedInsured
  }
  ...
  function removePolicyNamedInsured(polNamedInsured: PolicyNamedInsured) {
    if (not polNamedInsured.LocationNamedInsureds.IsEmpty) {
      throw new DisplayableException(DisplayKey.get("Web.Policy.LocationNamedInsured.UnexpectedDeletion", polNamedInsured, polNamedInsured.LocationNamedInsureds.first().Location))
    }
  ...
  function addNewPolicyBillingContactForContact(contact: Contact): PolicyBillingContact {
    return this.addNewPolicyContactRoleForContact(contact, TC_POLICYBILLINGCONTACT) as PolicyBillingContact
  }
  ...
  function changeBillingContactTo(contact: Contact) {
    ...
    if (pbcs.Count > 1) {
      throw new IllegalStateException(DisplayKey.get("Java.Contact.Error.DuplicateRoles", pbcs.Count, PolicyBillingContact, contact))
    }
    var pbc = pbcs.first()
    if (pbc == null) {
  ...
  property get PolicyAddressCandidates(): Address[] {
    return this.PrimaryNamedInsured.AccountContactRole.AccountContact.Contact.AllAddresses
        .subtract({this.PolicyAddress.Address})?.toTypedArray()
  }
  ...
  function mustInvalidateQuoteBeforeReleasingUWEditLock(authorityProfiles: UWAuthorityProfile[]): boolean {
    if (not this.ValidRate) {
      return false
    }
    else {
      return this.UWIssuesActiveOnly.issuesBlockingUser(this.ValidQuote ? TC_BLOCKSQUOTERELEASE : TC_BLOCKSRATERELEASE, authorityProfiles).HasElements
    }
  }
  ...
  protected property get HasAnyBlockingIssues(): boolean {
    return this.UWIssuesActiveOnly.hasMatch(\i -> i.CurrentBlockingPoint != TC_NONBLOCKING)
  }
  ...
  function getDiffItemsForDiffReason(reason: DiffReason, currentPeriod: PolicyPeriod): java.util.List<gw.api.diff.DiffItem> {
    if (reason == TC_MULTIVERSIONJOB or reason == TC_COMPAREJOBS) {
      return this.compareTo(reason, currentPeriod)
    } else if (reason == TC_POLICYREVIEW or reason == TC_INTEGRATION or reason == TC_APPLYCHANGES) {
  ...
  function clickPrintQuoteButton(templateType: String, fileName: String) {
    ...
    this.printDocument(templateType, fileName, DocumentsHelper.getPrintQuoteParameters())
  }

  /**
   * Renders to the given filename a PolicyPeriod in a printable format using the given template and parameters
   *
   * @param templateType the name of the template to use
   * @param fileName     the name of the file to generate
   * @param parameters   a map of symbols providing values for required symbols in the template
   */
  function printDocument(templateType: String, fileName: String, parameters: Map<String, Object>) {
    var templatePlugin = gw.plugin.Plugins.get(gw.plugin.document.IDocumentTemplateSource)
    if (templatePlugin == null) {
      throw new DisplayableException(DisplayKey.get("DocumentProduction.Error.PluginDisabled"))
    }
    ...
    if (template == null) {
      throw new DisplayableException(DisplayKey.get("DocumentProduction.Error.TemplateNotFound", templateType))
    }
    var docInfo = DocumentProduction.createDocumentSynchronously(
        template,
        parameters,
        new Document())
    DocumentsHelper.renderDocumentContentsWithDownloadDisposition(fileName, docInfo)
  }

  /**
   * Renders to a default filename a PolicyPeriod in a printable format using the given template and parameters
   *
   * @param templateType the name of the template to use
   * @param parameters   a map of symbols providing values for required symbols in the template
   */
  function printDocument(templateType: String, parameters: Map<String, Object>) {
    printDocument(templateType, templateType, parameters)
  }
  ...
  function onBeginIssueJob() {
    ...
    cleanUpLocationForRiskAssessment()
  }

  private function cleanUpLocationForRiskAssessment() {
    var associatedRiskAssessmentTempStore = Query.make(OutboundPeriodRiskAssessmentTempStore).compare(OutboundPeriodRiskAssessmentTempStore#PolicyPeriod, Equals, this).select()
    associatedRiskAssessmentTempStore.each(\tempStore -> {
      if (tempStore != null) {
        var removeTempStore = this.Bundle.add(tempStore)
        removeTempStore?.remove()
      }
    })
  }
  ...
  /**
   * Reset the denormalized financial fields associated with this PolicyPeriod.
   */
  function clearDenormalizedFinancialTotals() {
  ...
  function createReinsurables() {
    if (this.PolicyTerm.GenerateReinsurables) {
      ...
      removeReinsurables(this, this.AllReinsurables)
    }
  ...
  private function createReinsurablesFromRiskDatas(branch: PolicyPeriod, riskDatas: List<RiskData>) {
    ...
    removeReinsurables(branch, reinsurablesToRemove)
  ...
  private function removeReinsurables(branch: PolicyPeriod, reinsurablesToRemove: List<Reinsurable>) {
  ...
  /**
   * Reopens the PolicyPeriod for edit if it's currently rated or quoted.
   */
  function editIfRatedOrQuoted() {
    if (RatedOrQuoted) {
      JobProcess.edit()
    }
  }
  ...
  /**
   * Returns true if policy period is in rated or quoted status.
   */
  property get RatedOrQuoted(): boolean {
    var status = this.Status
    return status == TC_RATED || status == TC_QUOTED
  }

  /**
   * Determines if this policy period supports parallelization of availability during product
   * model synchronization. Default is true for all commercial lines that exceed the minimum number of coverables
   * configured to enable parallelization and false for all other lines.
   *
   * @return true if the PolicyPeriod supports parallelization, false if not
   */
  function shouldParallelizeProductModelAvailabilitySync(): boolean {
    return this.Policy.Product.Commercial and this.Lines.hasMatch(\line ->
        line.AllCoverables.Count >= PCConfigParameters.MinNumberOfCoverablesToParallelizeProductModelSync.Value)
  }
}
